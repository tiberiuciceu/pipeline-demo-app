...
export default function RootLayout({ children }: { readonly children: React.ReactNode }): React.ReactElement {
  const sidebarUserId = 'TC'
  const sidebarStatus = getPresenceStatus(sidebarUserId)
          ...
          <div className="border-t border-white/5 p-3">
            <div className="flex items-center gap-3 rounded-lg px-3 py-2">
              <div className="relative flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-500 text-xs font-bold text-white">
                TC
                <span
                  className={`absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full ring-2 ring-white ${presenceColors[sidebarStatus]}`}
                  aria-label={`${sidebarStatus}: ${sidebarUserId}`}
                />
              </div>
